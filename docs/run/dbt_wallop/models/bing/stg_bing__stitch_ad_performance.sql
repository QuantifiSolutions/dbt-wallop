  ...
  create or replace view `dbt-wallop-dev-1`.`bing`.`stg_bing__stitch_ad_performance`
  OPTIONS()
  as WITH
unioned_bing AS (
  SELECT * FROM `bigquery-312020`.`wallop_creative_bing_ads`.`campaign_performance_report`
  UNION ALL
  SELECT * FROM `bigquery-312020`.`wallop_creative_bing_ads_carmel`.`campaign_performance_report`
  UNION ALL
  SELECT * FROM `bigquery-312020`.`wallop_creative_bing_ads_hutton`.`campaign_performance_report`
  ),
